...
func TestIgnoreWildcardA(t *testing.T) {
	...
	assert.True(t, pattern.Matches("ns:gh-self/pre-commit/pre-commit"))

	pattern.MakeRelativePatternsAbsolute("my-hooks", "")
	assert.False(t, pattern.Matches("ns:gh-self/pre-commit/pre-commit"))
	assert.True(t, pattern.Matches("ns:my-hooks/pre-commitA/pre-commitC"))
	assert.True(t, pattern.Matches("ns:my-hooks/pre-commitB/pre-commitD"))

	pattern.AddPatterns("!ns:my-*/*/pre-commit*", "ns:gh-*/**/**/**/*pre-commit*")
	assert.True(t, pattern.Matches("ns:gh-self/pre-commit/pre-commit"))
	assert.False(t, pattern.Matches("ns:my-hooks/pre-commitA/pre-commitC"))
	assert.False(t, pattern.Matches("ns:my-hooks/pre-commitB/pre-commitD"))
}

func TestIgnoreWildcardB(t *testing.T) {
	pattern := HookPatterns{Patterns: []string{"**/pre-commit*/pre-commit*", "!**/pre-commit*/*"}}

	assert.False(t, pattern.Matches("pre-commit/pre-commit"))
