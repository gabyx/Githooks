...
"$GH_INSTALL_BIN_DIR/githooks-cli" shared add --shared file://"$GH_TEST_TMP/test112.shared/shared-repo.git" || exit 41
"$GH_INSTALL_BIN_DIR/githooks-cli" shared list | grep "shared-repo" | grep "'pending'" || exit 42
"$GH_INSTALL_BIN_DIR/githooks-cli" shared update || exit 43

if ! "$GH_INSTALL_BIN_DIR/githooks-cli" list | grep 'test-shared' | grep 'shared:repo' | grep "'active'" | grep "'untrusted'"; then
    "$GH_INSTALL_BIN_DIR/githooks-cli" list
    exit 5
fi

"$GH_INSTALL_BIN_DIR/githooks-cli" ignore add --pattern 'ns:mygagahooks/**/test-shared' ||
    exit 6

if ! "$GH_INSTALL_BIN_DIR/githooks-cli" list | grep 'test-shared' |
    grep "'shared:repo'" | grep "'ignored'" | grep -q "'untrusted'"; then
    echo "! Failed to ignore shared hook"
    exit 7
fi

"$GH_INSTALL_BIN_DIR/githooks-cli" ignore add --pattern '!ns:*/**/test-shared' ||
    exit 8
...
"$GH_INSTALL_BIN_DIR/githooks-cli" trust hooks --pattern 'ns:my*hooks/**/test-shared' ||
    exit 10
