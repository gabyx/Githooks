...
location2=$("$GH_INSTALL_BIN_DIR/githooks-cli" shared root-from-url "$url2") || exit 1

url3="ftp://github.com/test/repo3.git"
location3=$("$GH_INSTALL_BIN_DIR/githooks-cli" shared root-from-url "$url3") || exit 1
...
if ! "$GH_INSTALL_BIN_DIR/githooks-cli" list pre-commit | grep -q "'replaced'"; then
    echo "! Unexpected cli list output (1)"
    exit 1
fi

if ! "$GH_INSTALL_BIN_DIR/githooks-cli" list pre-commit | grep "shared-pre1" | grep -q "'shared:global'"; then
    echo "! Unexpected cli list output (2)"
    exit 1
fi

if ! "$GH_INSTALL_BIN_DIR/githooks-cli" list pre-commit | grep "local-pre" | grep "'repo'"; then
    echo "! Unexpected cli list output (3)"
    exit 1
fi

if ! "$GH_INSTALL_BIN_DIR/githooks-cli" list commit-msg | grep "'shared:global'" | grep -q "commit-msg"; then
    echo "! Unexpected cli list output (4)"
    exit 1
fi

if ! "$GH_INSTALL_BIN_DIR/githooks-cli" list post-commit | grep "local-post" | grep -q "'repo'"; then
    echo "! Unexpected cli list output (6)"
    exit 1
fi

if ! "$GH_INSTALL_BIN_DIR/githooks-cli" list post-commit | grep "'shared:repo'" | grep -q "post-commit"; then
    echo "! Unexpected cli list output (5)"
    exit 1
fi

if ! "$GH_INSTALL_BIN_DIR/githooks-cli" list post-merge | grep "'repo'" | grep -q "post-merge"; then
    echo "! Unexpected cli list output (7)"
    exit 1
fi

if ! "$GH_INSTALL_BIN_DIR/githooks-cli" list pre-push | grep "shared-pre2" | grep -q "'shared:repo'"; then
    echo "! Unexpected cli list output (8)"
    exit 1
fi

if ! "$GH_INSTALL_BIN_DIR/githooks-cli" list post-update | grep "shared-pre3" | grep -q "'shared:local'"; then
    echo "! Unexpected cli list output (9)"
    exit 1
fi

if ! "$GH_INSTALL_BIN_DIR/githooks-cli" list post-rewrite | grep "'shared:local'" | grep -q "'post-rewrite'"; then
    echo "! Unexpected cli list output (10)"
    exit 1
fi

if ! "$GH_INSTALL_BIN_DIR/githooks-cli" list | grep -q "Total.*hooks: '18'"; then
    echo "! Unexpected cli list output (12)"
    exit 1
fi

# Check all parallel batch names
OUT=$("$GH_INSTALL_BIN_DIR/githooks-cli" list --batch-name) || exit 11
...
root1=$("$GH_INSTALL_BIN_DIR/githooks-cli" shared root "ns:repo1")
...
root2=$("$GH_INSTALL_BIN_DIR/githooks-cli" shared root "ns:repo2")
...
root3=$("$GH_INSTALL_BIN_DIR/githooks-cli" shared root "ns:repo3")
