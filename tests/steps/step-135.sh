...
# Enable
"$GH_TEST_BIN/cli" config enable-containerized-hooks --local --set
if [ "$(git config --local githooks.containerizedHooksEnabled)" != "true" ]; then
    echo "Containerized not enabled."
    exit 1
fi

"$GH_TEST_BIN/cli" config enable-containerized-hooks --print |
    grep -iE ".*containerized.*enabled locally" ||
    die "Failed to enable"

"$GH_TEST_BIN/cli" config enable-containerized-hooks --global --set
if [ "$(git config --global githooks.containerizedHooksEnabled)" != "true" ]; then
    echo "Containerized not enabled."
    exit 1
fi

"$GH_TEST_BIN/cli" config enable-containerized-hooks --global --print |
    grep -iE ".*containerized.*enabled globally" ||
    die "Failed to enable"

# Disable
"$GH_TEST_BIN/cli" config enable-containerized-hooks --global --reset
...
"$GH_TEST_BIN/cli" config enable-containerized-hooks --local --reset
if [ "$(git config --local githooks.containerizedHooksEnabled)" != "" ]; then
    echo "Containerized not disabled."
    exit 1
fi

"$GH_TEST_BIN/cli" config enable-containerized-hooks --local --print |
