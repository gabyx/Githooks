...
OUT=$("$GH_INSTALL_BIN_DIR/githooks-cli" shared add --shared "$GH_TEST_TMP/shared/shared-clone.git" 2>&1)
...
OUT=$("$GH_INSTALL_BIN_DIR/githooks-cli" shared add --shared file://"$GH_TEST_TMP/shared/shared-clone.git" 2>&1)
# shellcheck disable=SC2181
if [ $? -eq 0 ] || ! echo "$OUT" | grep -q "You cannot add a URL"; then
...
# Test listing output
if "$GH_INSTALL_BIN_DIR/githooks-cli" shared list --shared | grep -q "shared-clone" ||
    "$GH_INSTALL_BIN_DIR/githooks-cli" shared list --all | grep -q "shared-clone"; then
    echo "! Expected to have an empty shared hooks list" >&2
    exit 1
fi

if ! "$GH_INSTALL_BIN_DIR/githooks-cli" shared add --global "$GH_TEST_TMP/shared/shared-clone.git"; then
    echo "! Expected adding local path to global shared hook repository to succeed" >&2
    exit 1
fi

if "$GH_INSTALL_BIN_DIR/githooks-cli" shared list --shared | grep -q "shared-clone" >/dev/null 2>&1 ||
    "$GH_INSTALL_BIN_DIR/githooks-cli" shared list | grep "shared-clone" | grep -qv "active"; then
...
"$GH_INSTALL_BIN_DIR/githooks-cli" shared remove --global "$GH_TEST_TMP/shared/shared-clone.git"
...
"$GH_INSTALL_BIN_DIR/githooks-cli" shared add --local "$GH_TEST_TMP/shared/shared-clone.git" || exit 1
"$GH_INSTALL_BIN_DIR/githooks-cli" shared add --global file://"$GH_TEST_TMP/shared/shared-server.git"@testbranch2 || exit 1
"$GH_INSTALL_BIN_DIR/githooks-cli" shared update || exit 1
"$GH_INSTALL_BIN_DIR/githooks-cli" shared list --all
...
"$GH_INSTALL_BIN_DIR/githooks-cli" shared add --global "$GH_TEST_TMP/shared/shared-clone.git" || exit 1
OUT=$(git commit --allow-empty -m "Test shared hooks" 2>&1)
