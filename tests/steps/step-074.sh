...
if ! "$GH_INSTALL_BIN_DIR/cli" list pre-commit | grep 'testing' | grep "'active'" | grep -q "'untrusted'"; then
    echo "! Unexpected list result (1)"
    exit 1
fi

if ! "$GH_INSTALL_BIN_DIR/cli" trust hooks --path "pre-commit/testing"; then
    echo "! Failed to accept the hook"
    exit 1
fi

if ! "$GH_INSTALL_BIN_DIR/cli" list pre-commit | grep 'testing' | grep "'active'" | grep -q "'trusted'"; then
