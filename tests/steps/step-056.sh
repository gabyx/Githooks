...
if ! "$GH_INSTALL_BIN_DIR/cli" ignore add --pattern "pre-commit/**"; then
    echo "! Failed to disable a hook"
    exit 1
fi
...
# Negate the pattern
if ! "$GH_INSTALL_BIN_DIR/cli" ignore add --pattern "!**/second"; then
    echo "! Failed to disable a hook"
    exit 1
fi
...
if ! "$GH_INSTALL_BIN_DIR/cli" ignore remove --all; then
